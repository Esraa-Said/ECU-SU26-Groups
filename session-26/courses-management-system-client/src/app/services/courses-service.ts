...
export class CoursesService {
  ...
  getCourseById(courseId: string): Observable<CourseInterface>{
    return this.httpClient.get<any>(`${this.baseUrl}/${courseId}`).pipe(
      map((res) => res.data.course)
    )
  }

  deleteCourse(courseId: string): Observable<CourseInterface>{
    return this.httpClient.delete<any>(`${this.baseUrl}/${courseId}`).pipe(
      map((res) => res.data.course)
    )
  }

  addCourse(course: FormData): Observable<CourseInterface>{
    return this.httpClient.post<any>(this.baseUrl, course).pipe(
      map((res) => res.data.course)
    )
  }

  updateCourse(course: CourseInterface, courseId: string): Observable<CourseInterface>{
    return this.httpClient.patch<any>(`${this.baseUrl}/${courseId}`, course).pipe(
      map((res) => res.data.course)
    )
  }
